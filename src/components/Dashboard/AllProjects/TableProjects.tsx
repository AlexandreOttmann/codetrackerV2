  ...
  return (
    <>
      ...
      <div className='container mx-auto h-max'>
        <div className='py-4'>
          <div>
            <h2 className='text-2xl text-custom-dark p-4 font-semibold tracking-wide'>Projets Github</h2>
          </div>
          <form>
            <div className='relative'>
              <div className='absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none'>
                <svg aria-hidden='true' className='w-5 h-5 text-[#979a9f] ' fill='none' stroke='currentColor' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'>
          ...
          <div className='-mx-4 sm:-mx-8 px-4 sm:px-8 py-5 overflow-x-auto'>
            <div className='inline-block min-w-full overflow-hidden'>
              <table className='min-w-full leading-normal'>
                <thead>
                  <tr>
                    <th className='pr-5 py-3 pl-10 border-b-2 font-light text-left text-sm text-gray-600 '>Projets</th>
                    <th className='px-5 py-3 border-b-2 font-light text-center text-sm text-gray-600 '>Language principal</th>
                    <th className='px-5 py-3 border-b-2 font-light text-center text-sm text-gray-600 '>Description</th>
                    <th className='px-5 py-3 border-b-2 font-light text-center text-sm text-gray-600 '>Dernier Update</th>
                  </tr>
                </thead>
                <tbody>
                                  ...
                                  <Link to={`/dashboard/${item.id}`} state={{ name: item.name }}>
                                    <div className='flex gap-2   md:justify-center  sm:items-center font-semibold'>
                                      ...
                                      <span className='text-gray-900 whitespace-no-wrap'>
                                        {item.name}
                                        <span className='text-xs text-custom-blue '>{item.fork ? `\nΨ` : null}</span>
                                        <p className='text-xs text-orange-300'>{dateConverter(item.created_at, true)}</p>
                                      </span>
                                    </div>
                                  </Link>
                                </div>
                                  ...
                                  <Link to={`/dashboard/${item.id}`} state={{ name: item.name }}>
                                    <div className='flex gap-2 justify-start md:justify-center items-center font-semibold'>
                                      ...
                                      <span className='text-gray-900 whitespace-no-wrap'>
                                        {item.name}
                                        <span className='text-xs text-custom-blue '>{item.fork ? `\nΨ` : null}</span>
                                        <p className='font-light text-sm text-gray-600'>{dateConverter(item.created_at, true)}</p>
                                      </span>
                                    </div>
                                  </Link>
                                </div>

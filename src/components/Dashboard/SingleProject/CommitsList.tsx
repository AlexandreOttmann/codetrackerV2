    ...
    <div className='container mx-auto'>
      <div className='py-4'>
        <div className='border-b mb-4'>
          <h2 className='text-2xl text-gray-600 p-4 font-semibold tracking-wide'>Liste des commits</h2>
        </div>
            ...
            <table className='min-w-full leading-normal'>
              <thead>
                <tr>
                  ...
                  <th className='px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider'>Nombres de changements</th>
                  ...
                  <tr key={item.sha}>
                    <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                      <Link to={`/dashboard/repos/commits/details/${item.sha}`} state={{ url: item.url }}>
                        <p className='text-gray-900 whitespace-no-wrap text-start'>{item.commit.message}</p>
                      </Link>
                    </td>
                    <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                      <div className='text-gray-900 whitespace-no-wrap flex pl-10 justify-start'>
                        <Link to={item.author.html_url} target='_blank'>
                          <div className='flex gap-2 justify-center items-center font-semibold'>
                            <img src={item.author.avatar_url} alt='avatar' className='w-7 h-7 rounded-full' />
